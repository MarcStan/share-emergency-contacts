...
namespace ShareEmergencyContacts.UserControls
{
    ...
    public class TemplateSelector : BindableObject
    {
        ...
        /// determine if a <see cref="NotSupportedException"/> is thrown when 
        ...
        /// <exception cref="NotSupportedException"></exception>Thrown if there is no datatemplate that matches the supplied type
        public DataTemplate TemplateFor(Type type)
        {
            ...
            if (template == null && ExceptionOnNoMatch)
                throw new NotSupportedException(type.ToString() + typesExamined);
            return template;
        }
